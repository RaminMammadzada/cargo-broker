...
export async function bootstrapTelegram(settingsRepository, { autoCreator } = {}) {
  ...
  let config = await readConfigFile(configUrl);

  const envToken = process.env.TELEGRAM_BOT_TOKEN?.trim();
  if (envToken) {
    await settingsRepository.persistBotToken(envToken);
    return { source: 'env', botToken: envToken, config, autoCreate: config.autoCreate };
  }

  const storedToken = await settingsRepository.getBotToken();
  if (storedToken) {
    process.env.TELEGRAM_BOT_TOKEN = storedToken;
    return { source: 'settings', botToken: storedToken, config, autoCreate: config.autoCreate };
  }

  if (config.botToken) {
    await settingsRepository.persistBotToken(config.botToken);
    process.env.TELEGRAM_BOT_TOKEN = config.botToken;
    return { source: 'config', botToken: config.botToken, config, autoCreate: config.autoCreate };
  }
    ...
    if (creator && typeof creator.ensureBotToken === 'function') {
      try {
        ...
        if (result?.botToken) {
          ...
          await writeConfigFile(configUrl, config);
          return {
            source: result.created ? 'auto-created' : 'auto-retrieved',
            botToken: result.botToken,
            config,
  ...
  return { source: 'none', botToken: null, config, autoCreate: config.autoCreate };
}
